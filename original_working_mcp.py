...
def _register_tools(server: FastMCP):
    ...
    async def query_codebase(
        ...
        selected_files = code_files[:max_results]
        ...
        if selected_files:
            ...
            prompt = f"The query '{query}' yielded no results in the codebase. The codebase may need to be indexed first."
    ...
    async def analyze_codebase_patterns() -> Dict[str, Any]:
        ...
        pattern_analysis = {
            ...
            "large_files": [
                f for f in py_files if f["size"] > 1000
            ],  # Files larger than 1KB
            "duplicate_imports": [],  # Will be populated with analysis
        }

        # Look for common patterns that might indicate optimization opportunities
        patterns_found = []
