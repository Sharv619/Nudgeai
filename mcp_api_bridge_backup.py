...
async def api_query_calendar(
    ...
    try:
        ...
        result = await mcp_server.call_tool(
            ...
            result = {
                "events": [],
                "insights": "Mock calendar data - MCP server tools not available",
                "summary": f"Mock data for {start_date} to {end_date}",
            }
...
async def api_generate_weekly_insights(start_date: Optional[str] = None):
    ...
    try:
        result = await mcp_server.tools["generate_weekly_insights"](
            start_date=start_date
        )
